<div class="row">
    <div class="col-md-12">
        <form action="{{ route('accounting.transaction.expense.update', $transaction->id) }}" method="post" id="expenseUpdateForm">
            @csrf
                ...
                <div class="erp-filter-item-wrapper filter-row d-flex flex-wrap align-items-center justify-content-between mb-3 ">
                    <div class="erp-filter-item flex-48">
                        <div class="input-block erp-step-input-block mb-0">
                            ...
                            <div class="cal-icon"><input class="form-control datetimepicker" name="date" value="{{ $transaction->transaction_date }}" required type="text"></div>
                                        ...
                                        @if(count($account_sub_category->accounts) > 0)
                                            <optgroup label="{{ $account_sub_category->name }}">
                                                @foreach($account_sub_category->accounts as $account)
                                                    <option value="{{ $account->id }}" {{ ($account->id == $transaction->account_id)?'selected':'' }}>{{ $account->name }}</option>
                                                @endforeach
                                            </optgroup>
                                        @endif
                                    @endforeach
                                        ...
                                        @if(count($expense_sub_category->accounts) > 0)
                                            <optgroup label="{{ $expense_sub_category->name }}">
                                                @foreach($expense_sub_category->accounts as $expense_account)
                                                    <option value="{{ $expense_account->id }}" {{ ($expense_account->id == $transaction->category_id)?'selected':'' }}>{{ $expense_account->name }}</option>
                                                @endforeach
                                            </optgroup>
                                        @endif
                                    @endforeach
                    ...
                    <div class="erp-filter-item flex-48">
                        <div class="input-block mb-0 erp-step-input-block ">
                            ...
                            <input type="text" class="form-control" name="amount" required min="1" placeholder="Enter Amount" value="{{ $transaction->total_amount }}">
                            ...
                            <select class="select select-step" name="vat_tax">
                                ...
                                @if(!empty($vat_taxes))
                                    @foreach($vat_taxes as $vat_tax)
                                        <option value="{{ $vat_tax->id }}" {{ in_array($vat_tax->id, $transaction_vat_tax_ids)?'selected':'' }}>{{ $vat_tax->name }} ( {{ $vat_tax->tax_rate }}% )</option>
                                    @endforeach
                                @endif
                    ...
                    <div class="erp-filter-item flex-100">
                        <div class="input-block mb-0 erp-step-input-block ">
                            ...
                            <textarea class="form-control" rows="3" name="description">{!! $transaction->description !!}</textarea>
