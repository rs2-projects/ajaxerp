...
@section('js')
    <script>
        ...
        $(document).ready(function(){
            ...
            $(document).on("submit", "#salarySetUpdateForm", function(e) {
                e.preventDefault();
                ...
                formPost(url, formData, function (res){
                    if(res.status == 200){
                        $("#edit_salary_set_modal").modal('hide');
        ...
        function editItem(id){
            let url = "{{route('hr.salary-set.edit', ':id')}}";
            url = url.replace(':id', id);
            ajaxGet(url, {}, function (response) {
                if (response.status == 200) {
                    $("#edit_salary_set_modal_body").html(response.view);
                    $("#edit_salary_set_modal").modal('show');
                    initializeSelect();
                } else {
