<div class="modal fade" id="printQrCodeModal" tabindex="-1" aria-labelledby="scanModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow">
            ...
            <form action="{{ route('inventory.boards.print-qr-code') }}" method="post" id="printQrCodeForm" target="_blank">
                @csrf
                                ...
                                @foreach($boards as $index => $board)
                                    <tr class="align-middle">
                                        <td class="text-center p-1">
                                            <input type="checkbox" name="item_id[{{ $index }}]" value="{{ $board->id }}">
                                        </td>
                                        <td class="p-1" style="white-space: normal;">{{ $board->name }}</td>
                                        <td class="p-1">
