...
@section('content')
    <!-- Start::row-1 -->
    <div class="row">
        @if(hasPermission('manage-finished-goods-category'))
...
@section('modals')
    @include('inventory.finished-good.finished-good-category._add_category_modal')
    @include('inventory.finished-good.finished-good-category._edit_category_modal')
@endsection
...
@section('js')
    <script>
        ...
        function getData(){
            getPaginatedListData("{{ route('inventory.finished-good-category.filtered') }}", "#ajax-data-load", filterData);
        }
        ...
        function editItem(id){
            let url = "{{route('inventory.finished-good-category.edit', ':id')}}";
