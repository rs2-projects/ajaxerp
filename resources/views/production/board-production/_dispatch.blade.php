    ...
    <div class="row" id="VueApp">
		<div class="erp-employee-list-wrapper">
			<div class="new-production-wrapper bg-card attd-table">
				<form action="{{route('production.board-production.dispatch.store', $pre_production->id)}}" id="dispatchStoreForm" method="post">
					@csrf
	...
	$("#dispatchStoreForm").submit(function(e) {
		...
		formPost(url, formData, function (res){
			if(res.status == 200){
				...
				setTimeout(function () {
					window.location.href = "{{route('production.board-production.index')}}";
				}, 1000);
			}else{
