                                    ...
                                    <div class="erp-filter-item flex-25">
                                        <div class=" form-focus select-focus custom-form-focus">
                                            <input type="text" class="form-control search-product-in"
                                                placeholder="Staff Title / User Name" id="keyword_filtered">
                                        </div>
                                    </div>
                                    <div class="erp-filter-item">
...
@section('modals')
    @include('production.production-staff._add_prod_staff_modal')
    @include('production.production-staff._edit_prod_staff_modal')
@endsection
...
@section('js')
    <script>
        ...
        $(document).ready(function() {
            ...
            $("#prodStaffStoreForm").on('submit', function (e) {
                var self = this;
                e.preventDefault();
            ...
            $(document).on("submit", "#prodStaffUpdateForm", function(e) {
                e.preventDefault();
        ...
        function getData(){
            getPaginatedListData("{{ route('production.production-staff.filtered') }}", "#ajax-data-load", filterData);
        }
        ...
        function editItem(id){
            let url = "{{route('production.production-staff.edit', ':id')}}";
            url = url.replace(':id', id);
            ajaxGet(url, {}, function (response) {
                if (response.status == 200) {
                    $("#edit_data_modal_body").html(response.view);
                    $("#editProdStaffModal").modal('show');
                } else {
