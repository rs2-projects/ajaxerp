...
@section('content')
    ...
    <div class="row">
            <div class="erp-add-employee-wrapper mb-3">
                <div class="erp-add-employee">
                    <a href="javascript:void(0)" class="btn add-btn erp-add-employee" data-bs-toggle="modal" data-bs-target="#addCustomerModal"><i class="fa-solid fa-plus"></i> New Customers</a>
    ...
    <div id="customerOtherContact" style="display: none;">
...
@section('modals')
    @include('sales.customer._add_customer_modal')
    @include('sales.customer._edit_customer_modal')
@endsection
...
@section('js')
    <script>
        ...
        $(document).ready(function() {
            ...
            $("#customerStoreForm").on('submit', function (e) {
                var self = this;
                e.preventDefault();
        ...
        function getData(){
            getPaginatedListData("{{ route('sales.customer.filtered') }}", "#ajax-data-load", filterData);
        }
        ...
        function editItem(id){
            let url = "{{route('sales.customer.edit', ':id')}}";
            url = url.replace(':id', id);
            ajaxGet(url, {}, function (response) {
                if (response.status == 200) {
                    $("#edit_customer_modal_body").html(response.view);
        ...
        function addOtherContacts(){
            var item = $('#customerOtherContact').html();
            $('.customerOtherContactContainer').append(item);
        }

        function removeOtherContact(element){
            $(element).closest('.customer-other-contact-parent').remove();
        }

        function getCountryWiseStates(select){
            let country_id = $(select).val();
            let url = "{{ route('sales.customer.get-states-by-country') }}";
